...
Observations -   
 ...
 Similarly, we can say that by creating an index for EmpID on course, and an index for ID on Employee reduces the time for the second query. 
...
The third query equated the EmpID from courses and ID from Employee, hence by creating a reference on these columns, it becomes faster to query.
